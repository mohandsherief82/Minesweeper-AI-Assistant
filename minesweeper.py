...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for cl in close_cells:
            if cl in self.mines: count_cpy -= 1
            if cl not in self.mines | self.safes: cells.add(cl)  # only add cells that are of unknown state
        new_sentence = Sentence(cells, count_cpy)  # prepare new sentence
        # add that sentence to knowledge only if it is not empty
        if len(new_sentence.cells) > 0: self.knowledge.append(new_sentence)
    ...
    def return_close_cells(self, cell):
        ...
        for rows in range(self.height):
            for columns in range(self.width):
                if abs(cell[0] - rows) <= 1 and abs(cell[1] - columns) <= 1 and (rows, columns) != cell: close_cells.add((rows, columns))
        return close_cells
